...
public class AppConfigure {
	...
	//flag : 1=生成token; 2=解析token; 3=返回所有信息;4=其他操作
	public static String tokenOperate(int flag, Object _infoOrToken){
		if(flag == 1){
			...
			Claim claim2 = claims.get("account");
			Claim claim1 = claims.get("organizationId");
			Claim claim3 = claims.get("userIndex");
			return claim1.asInt() + "-" + claim2.asString() + "-" + claim3.asString();
		}else{
			return "";
		}
	...
	@Bean
	public RegistServerEndpoint registServerEndpoint(){
		return new RegistServerEndpoint();
	}
